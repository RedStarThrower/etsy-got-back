 ...
 //Home example URL: https://openapi.etsy.com/v2/listings/active.js?includes=Images,Shop&api_key=aavnvygu0h5r52qes74x9zvo

 //Details example URL: load the item's data, i.e. with the unique id "183182778": https://openapi.etsy.com/v2/listings/183182778.js?api_key=aavnvygu0h5r52qes74x9zvo
 ...
 var ListView = Backbone.View.extend({
	...
	_render: function() { // <= promise handler
		...
		var resultsArray = this.model.get("results") //<= "get" gets into the attributes
		...
		htmlString += '<div class="seller-data">' + '<p class="seller">' + seller + '</p>' + '</div>'
		htmlString += '<div class="price-data">' + '<p class="price">' + "$" + price + '</p>' + '</div>'
		htmlString += '</div>'
		 }
...
var DetailView = Backbone.View.extend ({
	...
	_render: function() {
		...
		htmlString += '<div class="detail-seller-data">' + '<p class="detail-seller">' + seller + '</p>' + '</div>'
		htmlString += '<div class="detail-price-data">' + '<p class="detail-price">' + "$" + price + '</p>' + '</div>'
		htmlString += '</div>'
		this.el.innerHTML = htmlString
	}
})
 ...
 var SearchView = Backbone.View.extend({
	...
	events: { // <= Creates an event to switch views 
		"submit form": "_triggerSearchView"
	},

	_triggerSearchView: function(userQuery) {
		console.log(clickEvent.target) //<= testing; logs where the click happened
		window.location.hash = "search/" + userQuery
	},
